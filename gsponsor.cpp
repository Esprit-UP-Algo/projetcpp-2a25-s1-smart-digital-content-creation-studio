...
Gsponsor::Gsponsor(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::Gsponsor)
    ...
    , idSponsorEnCours(-1)
{
    ...
    connect(ui->tabWidget_4, &QTabWidget::currentChanged, this, &Gsponsor::on_tabWidget_4_currentChanged);
    connect(ui->tableWidget_4, &QTableWidget::cellDoubleClicked, this, &Gsponsor::on_tableWidget_4_cellDoubleClicked);
...
void Gsponsor::setEmbedded(bool embedded)
{
    m_embedded = embedded;
    if (m_embedded) {
        applyEmbeddedLayout();
        // En mode intégré dans Gemploye, afficher directement la page "Sponsor"
        // (équivalent au clic sur le bouton Employ_6)
        if (ui->stackedWidget)
            ui->stackedWidget->setCurrentIndex(3);
    }
}

void Gsponsor::applyEmbeddedLayout()
{
    // Cacher la barre latérale interne (doublon avec la barre principale de Gemploye)
    if (ui->frame) {
        ui->frame->hide();
    }

    // Utiliser un layout pour que le stackedWidget occupe tout l'espace disponible
...
void Gsponsor::sauvegarderStatuts()
{
    QSettings settings("SparkFlow", "GestionSponsors");
...
QString Gsponsor::getStatutSponsor(int codeSponsor)
{
...
void Gsponsor::setStatutSponsor(int codeSponsor, const QString &statut)
{
    statutsSponsors[codeSponsor] = statut;
    sauvegarderStatuts();
}

void Gsponsor::peuplerSecteursActivite()
{
    ui->comboBox_7->clear();
...
void Gsponsor::on_btn_microphone_clicked()
{
    lancer_analyse_strategique();
}

void Gsponsor::lancer_analyse_strategique()
{
...
QString Gsponsor::analyser_budget(const QString &requete)
{
...
QString Gsponsor::generer_conseil_strategique(int budget)
{
    ...
    for (const auto& secteur : secteurs) {
        ...
        query.prepare("SELECT COUNT(*), AVG(MONTANT) FROM SPONSOR WHERE SECTEUR_ACTIVITE = ?");
...
QVector<QPair<QString, double>> Gsponsor::analyser_secteurs()
{
    QVector<QPair<QString, double>> secteurs;

    QSqlQuery query("SELECT SECTEUR_ACTIVITE, AVG(MONTANT) FROM SPONSOR GROUP BY SECTEUR_ACTIVITE HAVING COUNT(*) > 0");
...
void Gsponsor::parler_reponse(const QString &reponse)
{
...
void Gsponsor::on_tabWidget_4_currentChanged(int index)
{
    if (index == 1) {
...
void Gsponsor::afficherStatistiques()
{
    QSqlQuery query;
...
void Gsponsor::afficherHistogrammeStatistiques()
{
    ...
    QSqlQuery query;
    query.exec("SELECT SECTEUR_ACTIVITE, COUNT(*), SUM(MONTANT) FROM SPONSOR GROUP BY SECTEUR_ACTIVITE");
...
double Gsponsor::calculerEchelle(double valeur)
{
...
double Gsponsor::calculerIntervalle(double maxValue)
{
...
void Gsponsor::setupSponsorsValidators()
{
...
bool Gsponsor::codeExistsInDatabase(int code)
{
    QSqlQuery query;
...
bool Gsponsor::isValidName(const QString &name)
{
...
bool Gsponsor::isValidSecteur(const QString &secteur)
{
    Q_UNUSED(secteur);
    return true;
}

bool Gsponsor::isValidMontantFormat(const QString &montant)
{
    QRegularExpression regex("^[0-9]+([.][0-9]{1,2})?$");
    return regex.match(montant).hasMatch();
}

void Gsponsor::highlightEmptyFields(const QString &code, const QString &nom)
{
    resetFieldStyles();
...
void Gsponsor::resetFieldStyles()
{
...
void Gsponsor::clearSponsorFields()
{
...
void Gsponsor::actualiserTableauSponsors()
{
...
void Gsponsor::chargerSponsorPourModification(int row)
{
...
void Gsponsor::annulerModification()
{
    clearSponsorFields();
    QMessageBox::information(this, "Annulation", "Mode modification annulé.");
}

void Gsponsor::on_tableWidget_4_cellDoubleClicked(int row, int column)
{
    Q_UNUSED(column);
    chargerSponsorPourModification(row);
}

void Gsponsor::on_Employ_clicked()
{
    ui->stackedWidget->setCurrentIndex(0);
}

void Gsponsor::on_Employ_4_clicked()
{
    ui->stackedWidget->setCurrentIndex(1);
}

void Gsponsor::on_Employ_2_clicked()
{
    ui->stackedWidget->setCurrentIndex(2);
}

void Gsponsor::on_Employ_6_clicked()
{
    ui->stackedWidget->setCurrentIndex(3);
}

void Gsponsor::on_Employ_3_clicked()
{
    ui->stackedWidget->setCurrentIndex(4);
}

void Gsponsor::on_Employ_5_clicked()
{
    ui->stackedWidget->setCurrentIndex(5);
}

void Gsponsor::on_pushButton_15_clicked()
{
...
void Gsponsor::on_pushButton_18_clicked()
{
...
void Gsponsor::on_pushButton_16_clicked()
{
    if (modeModification) {
        annulerModification();
    } else {
...
void Gsponsor::on_pushButton_17_clicked()
{
...
void Gsponsor::drawSectionHeader(QPainter &painter, const QString &title, int x, int y, int width)
{
...
void Gsponsor::on_lineEdit_25_textChanged(const QString &arg1)
{
    if(arg1.isEmpty()) {
        actualiserTableauSponsors();
    } else {
...
void Gsponsor::on_comboBox_5_currentTextChanged(const QString &arg1)
{
...
void Gsponsor::on_lineEdit_21_textChanged(const QString &text)
{
    if(text.isEmpty()) {
        ui->lineEdit_21->setStyleSheet("");
        return;
...
void Gsponsor::on_lineEdit_22_textChanged(const QString &text)
{
    if(text.isEmpty()) {
        ui->lineEdit_22->setStyleSheet("");
        return;
...
void Gsponsor::on_lineEdit_24_textChanged(const QString &text)
{
    if(text.isEmpty()) {
        ui->lineEdit_24->setStyleSheet("");
        return;
...
void Gsponsor::on_lineEdit_26_textChanged(const QString &text)
{
    if(text.isEmpty()) {
        ui->lineEdit_26->setStyleSheet("");
        return;
