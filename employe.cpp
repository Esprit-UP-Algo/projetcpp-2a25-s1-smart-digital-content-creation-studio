...
// Accès simplifié pour Arduino/Qt : récupérer le rôle à partir du CIN
QString employee::getRoleByCin(int cin)
{
    QSqlDatabase db = QSqlDatabase::database();
    if (!db.isOpen()) {
        qDebug() << "[DB] getRoleByCin - base non ouverte:" << db.lastError().text();
        return QString();
    }

    QSqlQuery query(db);
    query.prepare("SELECT ROLE FROM EMPLOYE WHERE CIN = :cin");
    query.bindValue(":cin", cin);

    if (!query.exec()) {
        qDebug() << "[DB] getRoleByCin - erreur SQL:" << query.lastError().text();
        return QString();
    }

    if (query.next()) {
        QString role = query.value(0).toString();
        qDebug() << "[DB] getRoleByCin - role trouve pour" << cin << ":" << role;
        return role;
    }

    qDebug() << "[DB] getRoleByCin - CIN inconnu:" << cin;
    return QString();
}
<<<<<<< Updated upstream
<<<<<<< Updated upstream
